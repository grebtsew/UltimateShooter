...
public class Game extends JComponent implements Runnable, MouseListener {
	...
	int skipvar = 0;
	BallTypes bt;
	HSComp hs = new HSComp();

	public Game(GameMode _gm, BallTypes _bt, int _level, int _time) {
	...
	private void init() {
		...
		case NORMAL:
			if (bt == BallTypes.FANTASY) {
				editor.fantasy = true;
			} else {
				editor.fantasy = false;
			}

			// fix a random awsome level
			editor.autocreate(2, 2, false);

			for (Map m : editor.getAutoMaps()) {
				maplist.add(m);
			}

			editor.autocreate(3, 3, false);

			for (Map m : editor.getAutoMaps()) {
				maplist.add(m);
			}

			editor.autocreate(4, 4, false);

			for (Map m : editor.getAutoMaps()) {
				maplist.add(m);
			}

			editor.autocreate(5, 5, false);

			for (Map m : editor.getAutoMaps()) {
				maplist.add(m);
			}

			editor.autocreate(6, 6, false);
	...
	public void update(Graphics g) {
		...
		// make sure the user knows whats happens
		if (GameOver) {
			gameStatus = "Game on!";
		} else {
			gameStatus = "Game Over!";
		}
		g.drawString(gameStatus, TIME_X - 100, TIME_Y);

		g.drawString("Time : " + timeLabel, TIME_X, TIME_Y);
		int temp = mapnumber - skipvar;
		g.drawString("Maps Completed : " + Integer.toString(temp), TIME_X,
				TIME_Y + DISP_Y);
		...
		while (GameOver) {
			...
			if (gamemode == GameMode.TIMER) {
				...
				if (timeLeft - elapsedSeconds <= 0) {
					...
					GameOver = false;
				   temp = mapnumber - skipvar;
					System.out.println("Game Over, Your score is : " + temp
							+ "!");
					gameend();
				}
			} else {
			...
			if (moving) {
				...
				case TIMER:
					...
					if (nextmap.ballsleft() == 1) {
						...
						int temp = mapnumber - skipvar;
						System.out.println("You finished map " + temp
								+ " and have "
								+ Math.round(timeLeft - elapsedSeconds)
								+ " seconds left.");
						nextmap();
	...
	private void gameend() {
		...
		case TIMER:
			hs.playerscore = temp;
			int place = hs.gettimeHSPlace();
			
			
			if(place == 0){
				// not highscore
				JOptionPane.showMessageDialog(this,
						"Game Over, Your score is : " + temp
								+ " in "+  Math.round(timeLeft) + " seconds!"
								+"\n" +
								"You did not make the top ten. Please try again!");
			} else {
			//  highscore
				...
				String s = (String)JOptionPane.showInputDialog(
				                    ...
				                    "Game Over, Your score is : " + temp
								+ " in "+  Math.round(timeLeft) + " seconds!" + 
				                    		"That puts you at place " + place + " of all time! Congratulations!" + "\n" +
								" Please write you name to register in the highscore!",
								"Customized Dialog",
				                    JOptionPane.PLAIN_MESSAGE,         
				                    null, null, null);	
				hs.playername = s;
				hs.addandsavetimeHS(null, 0, s, temp, place);
			}
